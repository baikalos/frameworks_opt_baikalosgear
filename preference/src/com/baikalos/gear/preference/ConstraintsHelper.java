 ...
 */
package com.baikalos.gear.preference;
...
import com.baikalos.gear.util.BaikalOSUtils;
...
public class ConstraintsHelper {
    ...
    public ConstraintsHelper(Context context, AttributeSet attrs, Preference pref) {
        ...
        TypedArray a = context.getResources().obtainAttributes(attrs,
                R.styleable.baikalos_SelfRemovingPreference);
        mReplacesKey = a.getString(R.styleable.baikalos_SelfRemovingPreference_replacesKey);
    ...
    private void checkIntent() {
        ...
        if (i != null) {
            if (!BaikalOSUtils.resolveIntent(mContext, i)) {
                Graveyard.get(mContext).addTombstone(mPref.getKey());
                mAvailable = false;
            }
        }
    ...
    private boolean checkConstraints() {
        ...
        try {
            ...
            boolean rOwner = a.getBoolean(R.styleable.baikalos_SelfRemovingPreference_requiresOwner, false);
            if (rOwner && UserHandle.myUserId() != UserHandle.USER_OWNER) {
                return false;
            }

            // Check if a specific package is installed
            String rPackageInst = a.getString(R.styleable.baikalos_SelfRemovingPreference_requiresPackageInstalled);
            if (rPackageInst != null) {
                boolean negated = isNegated(rPackageInst);
                if (negated) {
                    rPackageInst = rPackageInst.substring(1);
                }
                boolean available = BaikalOSUtils.isPackageInstalled(mContext, rPackageInst);
            ...
            String rPackageEnb = a.getString(R.styleable.baikalos_SelfRemovingPreference_requiresPackageEnabled);
            if (rPackageEnb != null) {
                boolean negated = isNegated(rPackageEnb);
                if (negated) {
                    rPackageEnb = rPackageEnb.substring(1);
                }
                boolean available = BaikalOSUtils.isPackageEnabled(rPackageEnb, mContext);
                if (available == negated) {
                    return false;
                }
            }
            ...
            String rPackageAvl = a.getString(R.styleable.baikalos_SelfRemovingPreference_requiresPackageAvailable);
            if (rPackageAvl != null) {
                boolean negated = isNegated(rPackageAvl);
                if (negated) {
                    rPackageAvl = rPackageAvl.substring(1);
                }
                boolean available = BaikalOSUtils.isPackageAvailable(rPackageAvl, mContext);
                if (available == negated) {
                    return false;
                }
            }
            ...
            String rAction = a.getString(R.styleable.baikalos_SelfRemovingPreference_requiresAction);
            if (rAction != null) {
                boolean negated = isNegated(rAction);
                if (negated) {
                    rAction = rAction.substring(1);
                }
                boolean available = BaikalOSUtils.resolveIntent(mContext, rAction);
                if (available == negated) {
                    return false;
                }
            }
            ...
            String rProperty = a.getString(R.styleable.baikalos_SelfRemovingPreference_requiresProperty);
            if (rProperty != null) {
            ...
            TypedValue tv = a.peekValue(R.styleable.baikalos_SelfRemovingPreference_requiresConfig);
            if (tv != null && tv.resourceId != 0) {
                if (tv.type == TypedValue.TYPE_STRING &&
                    ...
                    int mask = a.getInt(
                            R.styleable.baikalos_SelfRemovingPreference_requiresConfigMask, -1);
